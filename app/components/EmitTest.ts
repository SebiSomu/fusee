export interface EmitTestProps {
    initialCount?: number
}

export interface EmitTestResult {
    clickCount: SignalAccessor<number>
    lastEvent: SignalAccessor<string>
    emitSimple: () => void
    emitWithData: () => void
    emitObject: () => void
    template: string
...
export const EmitTest = defineComponent({
    setup(props: EmitTestProps, { emit }: { emit: (eventName: string, ...args: any[]) => void }): EmitTestResult {
        const clickCount = signal<number>(props.initialCount ?? 0)
        const lastEvent = signal<string>('None')
