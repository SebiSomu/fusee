...
func Banner() {
	fmt.Println()
	fmt.Printf("  %s%sFusée Framework %s  %sv2.4.0%s\n", BgCyan, Bold, Reset, FgGray, Reset)
...
func Select(title string, options []SelectOption, defaultIndex int) string {
	...
	render := func(firstTime bool) {
		if !firstTime {
			...
			fmt.Printf("\033[%dA", numOpts)
		}
		for i, opt := range options {
			fmt.Print("\033[2K\r") // Clear line and carriage return
			if i == selected {
				fmt.Printf("  %s❯%s %s%s%s", BoldCyan, Reset, BoldWhite, opt.Label, Reset)
				if opt.Desc != "" {
					fmt.Printf("  %s%s%s", FgGray, opt.Desc, Reset)
				}
			} else {
				fmt.Printf("    %s%s%s", FgGray, opt.Label, Reset)
				if opt.Desc != "" {
	...
	for {
		...
		if n == 1 {
			...
			case 3: // Ctrl+C
				...
				os.Exit(0)
			case 13, 10: // Enter
				// Erase the options lines and header
				fmt.Printf("\033[%dA", numOpts+1)
				for i := 0; i <= numOpts; i++ {
					fmt.Print("\033[2K\r\n")
				}
				fmt.Printf("\033[%dA", numOpts+1)
				// Print compact confirmed line:
				fmt.Printf("%s✔%s %s %s›%s %s%s%s\r\n", BoldGreen, Reset, title, FgGray, Reset, BoldCyan, options[selected].Label, Reset)
