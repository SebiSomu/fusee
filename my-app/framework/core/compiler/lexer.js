import { CompileError, throwError } from "./errors.js";
...
class Lexer {
    ...
    _readClosingTag() {
        ...
        if (!this._eof() && this._peek() === ">") {
            ...
            this._consume(">");
            this._push(TokenType.TAG_CLOSE, ">", closeStart);
        }
    }
    ...
    _readComment() {
        ...
        if (this._eof()) {
            throwError(
                ...
                createLoc(start, this._startPos()),
                this.source,
            );
        }
        this._consume("-->");
    ...
    _readMustache() {
        ...
        throwError(
            ErrorCode.UNTERMINATED_MUSTACHE,
            createLoc(openStart, this._startPos()),
            this.source,
            expr,
        );
    }
    ...
    _match(str) {
        if (this._lookingAt(str)) {
            for (const ch of str) this._advance();
            return true;
        }
        return false;
    }
