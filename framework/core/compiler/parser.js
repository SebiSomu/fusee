...
import { NodeType, createRoot, createElement, createComponent, createText, createInterpolation, createExpression, createAttribute, createBinding, createEvent, createDirective, createSlotOutlet, createSlotContent, createForArg, createLoc, createPos, } from './ast.js'

const VOID_ELEMENTS = new Set(['area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input', 'link', 'meta', 'param', 'source', 'track', 'wbr'])
const FOR_RE = /^(?:\(\s*([^,)]+?)\s*(?:,\s*([^)]+?)\s*)?\))\s+in\s+(.+)$|^([^\s,]+)\s+in\s+(.+)$/
...
class Parser {
    ...
    parse() {
        ...
        return createRoot(children, createLoc(start, this._startLoc(), this.source))
    }
    ...
    _parseMustache() {
        ...
        if (!expr.value.trim()) {
            throwError(ErrorCode.EMPTY_EXPRESSION, expr.loc, this.source, '{{ }}')
        }
    ...
    _parseElement() {
        ...
        const slotAttr = props.find(p => p.type === NodeType.ATTRIBUTE && p.name === 'slot')
        if (tag === 'template' && slotAttr) {
            return this._buildSlotContent(slotAttr, props, startLoc, selfClosing)
        }
        ...
        if (this._peek()?.type === TokenType.TAG_OPEN_CLOSE) {
            ...
            if (closeTok.value.toLowerCase() !== tag.toLowerCase()) {
                throwError(ErrorCode.UNEXPECTED_CLOSING_TAG, closeTok.loc, this.source, closeTok.value)
            }
            if (this._peek()?.type === TokenType.TAG_CLOSE) 
                this._advance()
        } else if (!this._eof()) {
            throwError(ErrorCode.MISSING_CLOSING_TAG, createLoc(startLoc, startLoc), this.source, tag)
        }
    ...
    _buildSlotOutlet(props, startLoc, selfClosing) {
        const nameAttr = props.find(p => p.type === NodeType.ATTRIBUTE && p.name === 'name')
        ...
        return createSlotOutlet(slotName, fallback, createLoc(startLoc, this._startLoc()))
    }
    ...
    _buildSlotContent(slotAttr, props, startLoc, selfClosing) {
        ...
        return createSlotContent(slotName, children, createLoc(startLoc, this._startLoc()))
    }
    ...
    _parseAttrs(tag) {
        ...
        while (!this._eof()) {
            const tok = this._peek()
            if (tok.type === TokenType.TAG_CLOSE || tok.type === TokenType.TAG_SELF_CLOSE || tok.type === TokenType.EOF) 
                break
            ...
            if (seen.has(rawName)) {
                throwError(ErrorCode.DUPLICATE_KEY, nameTok.loc, this.source, rawName, tag)
            }
            seen.add(rawName)
    ...
    _eof() {
        return this.pos >= this.tokens.length || this._peek()?.type === TokenType.EOF
    }
