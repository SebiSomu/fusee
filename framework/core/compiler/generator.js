...
class Generator {
    constructor(ast, options) {
        ...
        this._imports = new Set(['h', 'hText', 'createComponent', '_effect', '_batch'])
    }
    ...
    _genChildren(children) {
        if (!children || children.length === 0) 
            return []
        return children.map(c => this._genNode(c))
    }
    ...
    _genNode(node, inHoist = false) {
        ...
        switch (node.type) {
            ...
            default:
                throwError(ErrorCode.UNKNOWN_NODE_TYPE, node.loc, this.source, node.type)
        }
    }
    ...
    _genDirectiveEntries(dir, entries) {
        switch (dir.name) {
            ...
            case 'model': {
                ...
                entries.push(`'@input': { handler: ($e) => { _ctx.${expr}($e.target.value) }, modifiers: [] }`)
                break
            }
            ...
            default:
                entries.push(`${JSON.stringify('f-' + dir.name)}: ${dir.expression ? `() => ${this._wrapExpr(dir.expression.content)}` : 'true'}`)
        }
    }
    ...
    _genForNode(node, forDir) {
        ...
        return `hFor(\n    () => ${this._wrapExpr(source)},\n    (${itemParam}) => ${innerJs},\n    (${itemParam}) => ${this._wrapExpr(keyExpr)}\n  )`
    }

    _wrapExpr(expr) {
        if (expr.includes('_ctx.')) 
            return expr
        if (/^[a-zA-Z_$][a-zA-Z0-9_$.]*$/.test(expr.trim())) {
            const id = expr.trim()
