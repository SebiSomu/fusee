import { NodeType } from '../compiler/ast.js'
import { ErrorCode } from '../compiler/errors-list.js'
import { throwError } from '../compiler/errors.js'
...
class SSRGenerator {
    constructor(ast, options) {
        ...
        this._anchorIdx = 0
        this._imports = new Set(['escapeHtml', 'escapeAttr', 'renderClass', 'renderStyle', 'ssrEnumerate', 'renderComponentSSR', 'unref'])
    ...
    generate() {
        ...
        return [
            ...
            `export function renderSSR(_ctx, _components) {`,
            ...body.map(l => `    ${l}`),
            `}`
        ].join('\n')
    }
    ...
    _emitIf(node, out, varName) {
        ...
        node.branches.forEach((branch, i) => {
            const stmts = []
            if (branch.node) {
                const innerProps = (branch.node.props ?? []).filter(p => 
                    !(p.type === NodeType.DIRECTIVE && ['if', 'else-if', 'else'].includes(p.name))
                )
                const innerNode = { ...branch.node, props: innerProps }
                this._emitNode(innerNode, stmts, varName)
            }
    ...
    _emitComponent(node, out, varName) {
        ...
        out.push(`${varName} += renderComponentSSR(_components[${name}], ${props}, ${slots})`)
